...
router.post('/', requireAuth('ADMIN'), asyncHandler(async (req, res) => {
  ...
  await audit('User', user.id, 'CREATED', req.user.userId, null, req.user.tenantId);
...
router.patch('/:id', requireAuth('ADMIN'), asyncHandler(async (req, res) => {
  ...
  if (salaryAnnual !== undefined && String(salaryAnnual) !== String(existing.salaryAnnual)) {
    await audit('User', existing.id, 'SALARY_CHANGED', req.user.userId, `from ${existing.salaryAnnual} to ${salaryAnnual}`, req.user.tenantId);
  }
...
router.post('/:id/deactivate', requireAuth('ADMIN'), asyncHandler(async (req, res) => {
  const existing = await prisma.user.findFirst({ where: { id: req.params.id } });
  if (!existing) return res.status(404).json({ error: 'Employee not found' });
  if (existing.id === req.user.userId) return res.status(400).json({ error: 'Cannot deactivate yourself' });
  ...
  await audit('User', existing.id, 'DEACTIVATED', req.user.userId, null, req.user.tenantId);
  res.json({ ok: true });
}));
...
router.post('/:id/activate', requireAuth('ADMIN'), asyncHandler(async (req, res) => {
  ...
  await audit('User', existing.id, 'ACTIVATED', req.user.userId, null, req.user.tenantId);
  res.json({ ok: true });
}));
